...
assert.ok(home.includes('/release-ui.js?v=2026.09.21-responsive-shell-r2'),'taskbar JS must use a versioned URL to bypass an older controlling worker cache');
assert.ok(home.includes('/release-ui.css?v=2026.09.21-responsive-shell-r2'),'taskbar CSS must use a versioned URL to bypass an older controlling worker cache');
assert.match(sw,/\/layout-mode\.js/);
assert.match(sw,/\/admin-center\.html/);
assert.match(sw,/network-first|release-ui\.js/);

assert.equal(releaseId,'2026.09.21-responsive-shell-r2');
