export const SHADOW_FEATURE_VERSION='shadow-feature-extractor-v2';
...
export async function analyzeShadowFeatureCandidates(dataUrl,role='top'){
  ...
  if(!box||comp.area<Math.max(90,n*.012))return Object.freeze({
    schemaVersion:'aitc-shadow-feature-candidates-v2',runtimeVersion:SHADOW_FEATURE_VERSION,role:normalizedRole,status:'insufficient-roi',
  ...
  for(let y=box.minY;y<=box.maxY;y++){
    ...
    for(let x=box.minX;x<=box.maxX;x++){
      ...
      if(!isGlare){sr+=r/255;sg+=g/255;sb+=b/255;ss+=hsv.s;sv+=hsv.v;robustN++;if(((hsv.h>=275&&hsv.h<=345)||(b>g*1.04&&r>g*1.04))&&hsv.s>.15)purpleN++;}
  ...
  const roi={coverage:q(coverage),bboxAspect:q(box.width/Math.max(1,box.height)),centralized:q(1-Math.min(1,Math.abs(cx-w/2)/(w/2))),authority:false};
  ...
  if(normalizedRole==='top'){
    ...
    topCandidates={
      color:candidateColor(stats,flashRisk),
  ...
  return Object.freeze({
    schemaVersion:'aitc-shadow-feature-candidates-v2',runtimeVersion:SHADOW_FEATURE_VERSION,role:normalizedRole,status:'complete',
    authority:false,productionEligible:false,qc,roi,topCandidates,bottomCandidates,latencyMs:Math.round(performance.now()-started)
  });
}
