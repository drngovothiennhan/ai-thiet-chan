(()=>{
  ...
  const VERSION='preanalysis-image-enhancement-v2-audit';
  ...
  function outputSize(w,h){
    ...
    if(maxSide<800)scale=Math.min(MAX_UPSCALE,1440/maxSide);else if(maxSide<1280)scale=Math.min(1.5,1600/maxSide);else if(maxSide<1600)scale=1600/maxSide;
    scale=Math.max(1,scale);if(w*h*scale*scale>MAX_OUTPUT_PIXELS)scale=Math.sqrt(MAX_OUTPUT_PIXELS/(w*h));scale=Math.max(1,Math.min(MAX_UPSCALE,scale));
  ...
  async function enhanceDataUrl(dataUrl,qc={}){
    ...
    let drift=colorDrift(before.chroma,after.chroma),guard='pass',brightnessApplied=gamma<.999,rollback=false,rollbackReason=[];
    ...
    if(rollbackReason.length){
      enhanced=resizeMultipass(original,target.width,target.height);after=sampleMetrics(enhanced);drift=colorDrift(before.chroma,after.chroma);guard='resample-only';brightnessApplied=false;rollback=true;
    }
    return {dataUrl:toJpeg(enhanced),meta:{version:VERSION,enabled:true,nonGenerative:true,resampler:'canvas-high-quality-multipass',brightness:'hue-preserving-luminance-gamma',source:{width:w,height:h},output:{width:enhanced.width,height:enhanced.height,scale:Number(target.scale.toFixed(3))},brightnessApplied,gamma:Number(gamma.toFixed(3)),guard,rollback,rollbackReason,before:{brightness:Number(before.brightness.toFixed(1)),median:before.median,darkPct:Number((before.dark*100).toFixed(1)),glarePct:Number((before.glare*100).toFixed(1))},after:{brightness:Number(after.brightness.toFixed(1)),median:after.median,darkPct:Number((after.dark*100).toFixed(1)),glarePct:Number((after.glare*100).toFixed(1))},colorDrift:Number(drift.toFixed(4)),glareDeltaPct:Number(((after.glare-before.glare)*100).toFixed(2)),limits:{maxUpscale:MAX_UPSCALE,maxColorDrift:MAX_COLOR_DRIFT,maxGlareIncrease:MAX_GLARE_INCREASE,minGamma:MIN_GAMMA},elapsedMs:Math.round(performance.now()-started)}};
  }
  ...
  async function rewriteAnalyzeRequest(input,init){
    ...
    try{
      ...
      body.imageEnhancement={enabled:true,version:VERSION,configuredVersion:config.version,nonGenerative:true,colorIntegrityGuard:true};
