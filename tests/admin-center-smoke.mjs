...
for(const marker of ['/ui-controls.js?v=2.7.1','/admin-center.js?v=2.7.1','/admin-credentials.js?v=2.7.1']) assert.ok(settings.includes(marker),`missing settings loader: ${marker}`);
...
assert.ok(sw.includes("ai-thiet-chan-v2.7.1"),'PWA cache must be v2.7.1');
assert.ok(sw.includes("'/ui-controls.js'")&&sw.includes("'/admin-center.js'")&&sw.includes("'/admin-credentials.js'"),'Admin Center assets missing from PWA shell');
console.log('ADMIN CENTER SMOKE PASS: username/password login, forced first-login rotation, collapsed panels, diagnostics, backup and restore are wired');
