...
const [server,runtime,academic,corpus,sw,index]=await Promise.all([
  read('server.mjs'),read('runtime-guard.mjs'),read('academic-server.mjs'),read('knowledge-corpus.mjs'),read('public/sw.js'),read('public/index.html')
]);
...
console.log('PRE-RELEASE PIPELINE SMOKE PASS: AI fallback, 429 semantics, fusion/storage order, novelty learning gate, PSY1 boundary and UI clinical-limit wording are consistent.');
