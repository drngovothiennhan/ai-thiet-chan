...
assert.ok(sw.includes('ai-thiet-chan-v2.9.21-chat-gemini-admin'),'PWA cache version mismatch');
for(const asset of ['/runtime-stability.js','/diagnostic-worker-client.js','/vision-worker.js','/chat-flow-ui.js'])assert.ok(sw.includes(`'${asset}'`),`missing sustainable core asset: ${asset}`);
...
console.log('SUSTAINABLE RUNTIME SMOKE PASS: bounded worker vision, throttled observers, camera shutdown, admin-safe chat routing and minimal PWA lifecycle are wired.');