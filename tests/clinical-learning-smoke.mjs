// Repo-native regression gate for A.I Thiet Chan clinical learning v2.9.0.
...
for(const marker of [
  'HIU CLB YHCT','ai_thiet_chan_submit_feedback_v1','ai_thiet_chan_admin_list_feedback_v1',
  'ai_thiet_chan_admin_review_feedback_v1','ai_thiet_chan_find_learned_cases_v1','Góp ý ca lâm sàng',
  'Phân tích ca trước','clinical-feedback-card{display:block}','Bác sĩ','Y sĩ','admin duyệt'
...
assert.ok(sw.includes("ai-thiet-chan-v2.9.8-knowledge-5doc-complete"),'service worker compatibility marker missing');
assert.ok(settings.includes("'/feedback-lifecycle.js','aitcFeedbackLifecycle'")&&settings.includes('afterWindowLoad(async()=>'),'feedback lifecycle must remain post-load');
