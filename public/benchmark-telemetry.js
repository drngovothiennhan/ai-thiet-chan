...
function install(){
  ...
  window.addEventListener('aitc:vision-shadow',event=>{
    ...
    persistShadow({
      ...
      shadowPreprocessVersion:String(shadow?.topFeatures?.preprocess?.version||''),
      shadowModelGuidedRoi:Boolean(shadow?.topFeatures?.preprocess?.modelGuidedRoi),
      shadowNeutralReferencePixels:Number.isFinite(Number(shadow?.topFeatures?.preprocess?.neutralReferencePixels))?Number(shadow.topFeatures.preprocess.neutralReferencePixels):null,
      shadowNormalizationApplied:Boolean(shadow?.topFeatures?.preprocess?.normalization?.applied),
      shadowNormalizationReason:String(shadow?.topFeatures?.preprocess?.normalization?.reason||''),
      shadowGainR:Number.isFinite(Number(shadow?.topFeatures?.preprocess?.normalization?.gains?.r))?Number(shadow.topFeatures.preprocess.normalization.gains.r):null,
      shadowGainG:Number.isFinite(Number(shadow?.topFeatures?.preprocess?.normalization?.gains?.g))?Number(shadow.topFeatures.preprocess.normalization.gains.g):null,
      shadowGainB:Number.isFinite(Number(shadow?.topFeatures?.preprocess?.normalization?.gains?.b))?Number(shadow.topFeatures.preprocess.normalization.gains.b):null,
      shadowRoiCentralized:Number.isFinite(Number(shadow?.roiGeometry?.centralized))?Number(shadow.roiGeometry.centralized):null,
      shadowRoiTouchesFrame:typeof shadow?.roiGeometry?.touchesFrame==='boolean'?shadow.roiGeometry.touchesFrame:null,
      ...
      topMoistureLabelCandidate:String(shadow?.topFeatures?.topCandidates?.moisture?.labelCandidate||''),
      topMoistureGlossRatio:Number.isFinite(Number(shadow?.topFeatures?.topCandidates?.moisture?.surfaceHighlightRatio))?Number(shadow.topFeatures.topCandidates.moisture.surfaceHighlightRatio):null,
      topMoistureStrictGlossRatio:Number.isFinite(Number(shadow?.topFeatures?.topCandidates?.moisture?.strictGlossRatio))?Number(shadow.topFeatures.topCandidates.moisture.strictGlossRatio):null,
      topMoistureDistributedGlossRatio:Number.isFinite(Number(shadow?.topFeatures?.topCandidates?.moisture?.distributedGlossRatio))?Number(shadow.topFeatures.topCandidates.moisture.distributedGlossRatio):null,
      topMoistureLargestGlossComponentRatio:Number.isFinite(Number(shadow?.topFeatures?.topCandidates?.moisture?.largestGlossComponentRatio))?Number(shadow.topFeatures.topCandidates.moisture.largestGlossComponentRatio):null,
      topMoistureRoughness:Number.isFinite(Number(shadow?.topFeatures?.topCandidates?.moisture?.roughness))?Number(shadow.topFeatures.topCandidates.moisture.roughness):null,
      topMoistureOverexposedRatio:Number.isFinite(Number(shadow?.topFeatures?.topCandidates?.moisture?.overexposedRatio))?Number(shadow.topFeatures.topCandidates.moisture.overexposedRatio):null,
      topMoistureSampledPixels:Number.isFinite(Number(shadow?.topFeatures?.topCandidates?.moisture?.sampledPixels))?Number(shadow.topFeatures.topCandidates.moisture.sampledPixels):null,
