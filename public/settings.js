(()=>{
  ...
  function ensureInstallUi(){
    ...
    const group=document.createElement('div');group.className='settings-group';group.id='pwaInstallGroup';
    ...
    const meta=panel.querySelector('.settings-meta');if(meta)panel.insertBefore(group,meta);else panel.appendChild(group);
    document.getElementById('pwaInstallBtn')?.addEventListener('click',installApp);updateInstallUi();
  }
  function updateInstallUi(message=''){
    const btn=document.getElementById('pwaInstallBtn'),status=document.getElementById('pwaInstallStatus');if(!btn||!status)return;
    if(isStandalone()){btn.textContent='Đã cài đặt';btn.disabled=true;status.textContent='A.I Thiệt Chẩn đang chạy ở chế độ ứng dụng độc lập.';return;}
    btn.disabled=false;btn.textContent=deferredInstallPrompt?'Cài đặt ngay':'Cài đặt A.I Thiệt Chẩn';
  ...
  async function installApp(){
    ...
    if(deferredInstallPrompt){const prompt=deferredInstallPrompt;deferredInstallPrompt=null;try{await prompt.prompt();const choice=await prompt.userChoice;updateInstallUi(choice?.outcome==='accepted'?'Đã xác nhận cài đặt. Ứng dụng sẽ xuất hiện trên thiết bị.':'Bạn đã hủy cài đặt. Có thể thử lại từ menu trình duyệt.');}catch{updateInstallUi(installFallbackText());}return;}
    updateInstallUi(installFallbackText());
  }
  window.addEventListener('beforeinstallprompt',event=>{event.preventDefault();deferredInstallPrompt=event;ensureInstallUi();updateInstallUi();});
  window.addEventListener('appinstalled',()=>{deferredInstallPrompt=null;updateInstallUi('Đã cài A.I Thiệt Chẩn thành công.');});

  function readMode(){try{return localStorage.getItem(STORAGE_KEY)==='general'?'general':'normal';}catch{return 'normal';}}
  ...
  openBtn.addEventListener('click',open);closeBtn?.addEventListener('click',close);normalSetting.addEventListener('click',()=>applyMode('normal'));generalSetting.addEventListener('click',()=>applyMode('general'));
  dialog.addEventListener('click',e=>{if(e.target===dialog)close();});dialog.addEventListener('cancel',e=>{e.preventDefault();close();});
  ensureInstallUi();const initial=readMode();paint(initial);queueMicrotask(()=>applyMode(initial));
})();
...
(()=>{
  ...
  if(!document.querySelector('script[data-aitc-ui-controls]')){const script=document.createElement('script');script.src='/ui-controls.js?v=2.7.0';script.async=false;script.dataset.aitcUiControls='true';document.head.appendChild(script);}
  if(!document.querySelector('script[data-aitc-admin-center]')){const script=document.createElement('script');script.src='/admin-center.js?v=2.7.0';script.async=false;script.dataset.aitcAdminCenter='true';document.head.appendChild(script);}
})();
