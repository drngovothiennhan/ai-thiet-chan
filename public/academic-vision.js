(function(scope){
'use strict';
const SOURCE=Object.freeze({id:'KNOWLEDGE-5DOC',sourceCount:5,indexedPages:1157,indexedImageOccurrences:1027,pageVisualSignatures:298,imageVisualSignatures:298,knowledgeVersion:'thiet-chan-kb-2026-09-15.5doc',noSilentOmission:true,segmentationVersion:'adaptive-tongue-mask-v2'});
...
async function signatureFromDataUrl(dataUrl){
  ...
  try{
    ...
    if(den<50)return null;const q=v=>Number(v.toFixed(4));return {r:q(sr/den),g:q(sg/den),b:q(sb/den),s:q(ss/den),v:q(sv/den),purple:q(purple/den),white:q(white/den),yellow:q(yellow/den),dark:q(dark/den),spot:q(spot/den),aspect:q(bw/bh),coverage:q(comp.area/n),segmentationMode};
