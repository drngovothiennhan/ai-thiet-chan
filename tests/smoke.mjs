...
try{
  ...
  requireMarkers(sw,['ai-thiet-chan-v2.9.8-knowledge-5doc-complete',"url.pathname.startsWith('/api/')",'/release-ui.js'],'service worker');
  for(const secondary of ['/admin-center.js','/admin-credentials.js','/ui-controls.js','/feedback-lifecycle.js','/upload-controls.js','/torch.js'])if(sw.includes(`'${secondary}'`))throw new Error(`secondary asset must not block service-worker install: ${secondary}`);
  if(sw.includes('skipWaiting()')||sw.includes('clients.claim()'))throw new Error('service worker must not force mid-session takeover');
  ...
  console.log('SMOKE PASS: production shell, dual-view AI, approved learning, lazy operational modules and nonblocking PWA startup are wired');
